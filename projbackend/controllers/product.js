...
const fs = require("fs") //file system to access it
const product = require('../models/product')
...
exports.createProduct=(req,res)=>{
    ...
    form.parse(req,(err,fields,file)=>{
        ...
        product.save((err,product)=>{
            if(err){
                return res.status(400).json({error:"saving prduct  to DB failed"})
            }
            res.json(product)
        })
...
exports.deleteProduct=(req,res)=>{
    ...
    product.remove((err,deletedProduct)=>{
        ...
        res.json({message:"Product deleted from DB",deletedProduct})
    })
}
...
exports.updateStock=(req,res,next)=>{
    ...
    product.bulkWrite(myOperation,{},(err,products)=>{
      if(err){
        return res.status(400).json({error:"Buk operation faild"})  
      }
      next()
    })
}
